...
sim_param.N = 11; % TOTALE NODI
sim_param.K = 6; % ONESTI
...
for pmal_dec = 1:1:6
    ...
    [results] = function_fixed_states_optimized(sim_param);
...
%     Hard_N_11_T_9(pmal_dec) = min(PFAr + 1-PDr);
%     Soft_N_11_T_9(pmal_dec) = min(PFAr_LLR+1-PDr_LLR);
%     %==========END OF RESULTS COMPUTATIONS OF VARSHNEY AND LLR========================================
%     perr_majority(pmal_dec) = results.error_majority;
%         
     fixed_states_N_11_T_9(pmal_dec,:)=  results.p_err; % this is only for max entropy game result
    
    
end

% save('fixed_states_N_11_T_9_1st_row.mat','fixed_states_N_11_T_9_1st_row');